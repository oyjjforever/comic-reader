<template>
  <div class="home-container" :class="{ 'dark-mode': isDarkMode }">
    ...
    <!-- 主体内容区域 -->
    <main class="main-content" v-if="videoResourcePath">
      <!-- 左侧文件树 -->
...
const fetchTreeData = async () => {
  if (!videoResourcePath.value) return
  isLoading.value = true
  try {
    // 并行加载树形数据和平铺数据
    const treeData = await window.video.getFolderTree(videoResourcePath.value)
    tree.data = [
      {
        name: '我的收藏',
      ...
      { name: '资源目录', fullPath: videoResourcePath.value, children: treeData }
    ]
    onTreeNodeClick(videoResourcePath.value as string)
  } catch (error: any) {
...
const onTreeNodeClick = async (folderPath: string) => {
  ...
  try {
    grid.filterRows = grid.rows = await window.video.getFiles(folderPath)
