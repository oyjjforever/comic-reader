import { tags } from "@/typings/database";
import { Database } from 'sqlite'
import database from "./sqlite";

// 打开数据库
let db: Database | null = null;

/**
 * @description: 获取所有标签
 * @param {string} order 排序方式，例如`id DESC, id ASC`
 * @return {Promise<tags[]>} 返回标签列表
 */
const getTags = async (order?: string): Promise<tags[]> => {
    if (!db) {
        db = await database.openDatabase()
    }

    if (!order) {
        order = 'id DESC'
    }

    const sql = `SELECT * FROM tags ORDER BY ${order}`
    return await db.all<tags[]>(sql)
}

/**
 * @description: 根据ID获取单个标签
 * @param {number} id 标签ID
 * @return {Promise<tags>} 返回标签信息
 */
const getTag = async (id: number): Promise<tags> => {
    if (!db) {
        db = await database.openDatabase()
    }

    try {
        const tag = await db.get<tags>('SELECT * FROM tags WHERE id = ?', id)

        if (!tag) {
            throw new Error('标签不存在')
        }

        return tag
    } catch (error) {
        throw error
    }
}

/**
 * @description: 根据标签名称获取标签
 * @param {string} label 标签名称
 * @return {Promise<tags | null>} 返回标签信息或null
 */
const getTagByLabel = async (label: string): Promise<tags | null> => {
    if (!db) {
        db = await database.openDatabase()
    }

    try {
        const tag = await db.get<tags>('SELECT * FROM tags WHERE label = ?', label)
        return tag || null
    } catch (error) {
        throw error
    }
}

/**
 * @description: 添加标签
 * @param {string} label 标签名称
 * @return {Promise<number>} 返回标签ID
 */
const addTag = async (label: string): Promise<number> => {
    if (!db) {
        db = await database.openDatabase()
    }

    try {
        // 检查标签是否已存在
        const existingTag = await getTagByLabel(label)
        if (existingTag) {
            throw new Error('该标签已存在')
        }

        const result = await db.run(`
            INSERT INTO tags (label, created_at)
            VALUES (?, ?)
        `, label, new Date())

        if (!result.lastID) {
            throw new Error('添加标签失败')
        }

        return result.lastID
    } catch (error) {
        throw error
    }
}

/**
 * @description: 更新标签
 * @param {number} id 标签ID
 * @param {string} label 新的标签名称
 * @return {Promise<void>} 返回更新结果
 */
const updateTag = async (id: number, label: string): Promise<void> => {
    if (!db) {
        db = await database.openDatabase()
    }

    try {
        // 检查标签是否存在
        const existingTag = await getTag(id)
        if (!existingTag) {
            throw new Error('标签不存在')
        }

        // 检查新标签名称是否已被其他标签使用
        const tagWithSameLabel = await getTagByLabel(label)
        if (tagWithSameLabel && tagWithSameLabel.id !== id) {
            throw new Error('该标签名称已被使用')
        }

        const result = await db.run('UPDATE tags SET label = ? WHERE id = ?', label, id)

        if (!result.changes) {
            throw new Error('更新标签失败')
        }
    } catch (error) {
        throw error
    }
}

/**
 * @description: 删除标签
 * @param {number} id 标签ID
 * @return {Promise<void>} 返回删除结果
 */
const deleteTag = async (id: number): Promise<void> => {
    if (!db) {
        db = await database.openDatabase()
    }

    try {
        // 检查标签是否存在
        const existingTag = await getTag(id)
        if (!existingTag) {
            throw new Error('标签不存在')
        }

        // 获取所有使用该标签的收藏
        const favoritesWithThisTag = await db.all<{ id: number, tags: string }[]>('SELECT id, tags FROM favorites WHERE tags LIKE ?', `%${id}%`)

        // 从所有收藏中移除该标签ID
        for (const favorite of favoritesWithThisTag) {
            if (favorite.tags) {
                const tagIds = favorite.tags.split(',').filter(tagId => tagId.trim() !== '' && tagId !== id.toString())
                const newTags = tagIds.join(',')
                await db.run('UPDATE favorites SET tags = ? WHERE id = ?', newTags, favorite.id)
            }
        }

        // 删除标签
        const result = await db.run('DELETE FROM tags WHERE id = ?', id)

        if (!result.changes) {
            throw new Error('删除标签失败')
        }
    } catch (error) {
        throw error
    }
}

/**
 * @description: 获取标签数量
 * @return {Promise<number>} 返回标签总数
 */
const getTagCount = async (): Promise<number> => {
    if (!db) {
        db = await database.openDatabase()
    }

    try {
        const result = await db.get<{ count: number }>('SELECT COUNT(*) as count FROM tags')
        return result?.count || 0
    } catch (error) {
        throw error
    }
}

/**
 * @description: 根据ID数组获取标签列表
 * @param {number[]} ids 标签ID数组
 * @return {Promise<tags[]>} 返回标签列表
 */
const getTagsByIds = async (ids: number[]): Promise<tags[]> => {
    if (!db) {
        db = await database.openDatabase()
    }

    if (ids.length === 0) {
        return []
    }

    try {
        const placeholders = ids.map(() => '?').join(',')
        const sql = `SELECT * FROM tags WHERE id IN (${placeholders}) ORDER BY id`
        return await db.all<tags[]>(sql, ...ids)
    } catch (error) {
        throw error
    }
}

export default {
    getTags,
    getTag,
    getTagByLabel,
    addTag,
    updateTag,
    deleteTag,
    getTagCount,
    getTagsByIds,
}