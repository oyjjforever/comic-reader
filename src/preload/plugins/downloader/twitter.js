...
async function getVideoUrls(tweetId) {
  const apiUrl = 'https://twitter.com/i/api/graphql/0hWvDhmW8YQ-S_ib3azIrw/TweetResultByRestId'
  const variables = {
    tweetId: tweetId,
    withCommunity: false,
    includePromotedContent: false,
  ...
  const headers = await buildHeader()
  const res = await api.get({
    url: `${apiUrl}?variables=${encodeURIComponent(JSON.stringify(variables))}&features=${encodeURIComponent(JSON.stringify(features))}`,
    headers
  })
  return (
